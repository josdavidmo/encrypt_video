...
while(True):
    ...
    encrypt_encrypt_image[row][column] = (
        encrypt_encrypt_image[row][column] + np.random.randint(0, 255, 1)[0]) % 256
    ...
    for i, col in enumerate(color):
        ...
        npcr = float(num_pixels - np.count_nonzero(np.equal(
            encrypt_image[:, :, i], encrypt_encrypt_image[:, :, i]))) / float(num_pixels)
        print "--- NPCR %s %s ---" % (col, npcr * 100)
        uaci = float(np.sum(
            abs((frame[:, :, i] - encrypt_image[:, :, i]) / 255))) / float(num_pixels)
